...
func (con *Connector) CreateQueue(name string, durable bool, priority int) error {
	...
	con.Logger.Infof("[RabbitMQ] Create queue (%s), durable: %v, priority: %d", name, durable, priority)
...
func (con *Connector) DeleteQueue(name string) error {
	...
	con.Logger.Infof("[RabbitMQ] Delete queue (%s), remain messages: %d", name, total)
	// Success
	return err
}
...
func (con *Connector) BindQueueToExchange(queue, exchange, key string) error {
	// Success
	con.Logger.Infof("[RabbitMQ] Bind queue (%s) to exchange (%s) with key: %s", queue, exchange, key)
...
func (con *Connector) UnbindQueueToExchange(queue, exchange, key string) error {
	// Success
	con.Logger.Infof("[RabbitMQ] Unbind queue (%s) to exchange (%s) with key: %s", queue, exchange, key)
